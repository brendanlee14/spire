...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const strategy = getStrategy("sts1", cSlug, sSlug);
  const character = getCharacter("sts1", cSlug);
  if (!strategy || !character) return {};
  return { title: `${strategy.name} — ${character.name} — Slay the Spire` };
}
    ...
    <main className="max-w-3xl mx-auto">
      <GameHeader
        crumbs={[
          ...
          { label: character.name, href: `/sts1/${character.slug}` },
          { label: strategy.name },
        ]}
